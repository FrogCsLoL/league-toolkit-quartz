...
fn sanitize_str(s: &str) -> Value {
    if s == "true" { return Value::Int(1); }
    if s == "false" { return Value::Int(0); }
    if s.eq_ignore_ascii_case("nan") { return Value::Float(f64::NAN); }
...
fn read_numbers(
    ...
    for j in 0..num {
        ...
        result.push(RawEntry { hash: keys[j], value, storage });
    }
    Ok(result)
}
