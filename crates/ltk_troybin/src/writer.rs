...
pub fn write_v2(entries: &[RawEntry]) -> Result<Vec<u8>, TroybinError> {
    ...
    let string_pool = if !buckets[12].is_empty() {
        ...
        StringPool { offsets: Vec::new(), data: Vec::new() }
    };
    ...
    for bit in 0u16..14 {
        ...
        match bit {
            ...
            _ => write_number_block(&mut out, &buckets[bit as usize], StorageType::from_bit(bit).unwrap()),
        }
    }
...
fn write_bool_block(out: &mut Vec<u8>, entries: &[&RawEntry]) {
    ...
    // Packed booleans
    let bytes_count = (entries.len() / 8) + if entries.len() % 8 > 0 { 1 } else { 0 };
...
fn write_component(out: &mut Vec<u8>, raw: f64, storage: StorageType) {
    match storage {
        ...
        StorageType::Float32 | StorageType::Float32x2
        | StorageType::Float32x3 | StorageType::Float32x4 => {
            out.extend_from_slice(&(raw as f32).to_le_bytes());
        }
        StorageType::U8Scaled | StorageType::U8 | StorageType::U8x2Scaled
        | StorageType::U8x3Scaled | StorageType::U8x4Scaled => {
...
pub fn write_binary(entries: &[RawEntry]) -> Result<Vec<u8>, TroybinError> {
    ...
    if all_old && !entries.is_empty() {
        return Err(TroybinError::IniParse {
            line: 0,
            message: "Cannot write v1 (old format) entries to binary. Convert to v2 storage types first.".to_string(),
        });
    }
    write_v2(entries)
}
...
mod tests {
    ...
    fn round_trip_numbers() {
        let entries = vec![
            RawEntry { hash: 100, value: Value::Int(42), storage: StorageType::Int32 },
            RawEntry { hash: 200, value: Value::Float(3.14), storage: StorageType::Float32 },
            RawEntry { hash: 300, value: Value::Vec(vec![1.0, 2.0, 3.0]), storage: StorageType::Float32x3 },
        ];
    ...
    fn round_trip_bools() {
        let entries = vec![
            RawEntry { hash: 600, value: Value::Int(1), storage: StorageType::Bool },
            RawEntry { hash: 700, value: Value::Int(0), storage: StorageType::Bool },
            RawEntry { hash: 800, value: Value::Int(1), storage: StorageType::Bool },
        ];
        let bytes = write_v2(&entries).unwrap();
        let (_, read_back) = read_binary(&bytes).unwrap();
        assert_eq!(read_back.len(), 3);
        match &read_back[0].value { Value::Int(v) => assert_eq!(*v, 1), _ => panic!() }
        match &read_back[1].value { Value::Int(v) => assert_eq!(*v, 0), _ => panic!() }
        match &read_back[2].value { Value::Int(v) => assert_eq!(*v, 1), _ => panic!() }
    }

    #[test]
    fn round_trip_mixed() {
        let entries = vec![
            RawEntry { hash: 10, value: Value::Int(7), storage: StorageType::Int32 },
            RawEntry { hash: 20, value: Value::Float(1.5), storage: StorageType::Float32 },
            RawEntry { hash: 30, value: Value::Int(1), storage: StorageType::Bool },
            RawEntry { hash: 40, value: Value::String("test.dds".to_string()), storage: StorageType::StringBlock },
            RawEntry { hash: 50, value: Value::Vec(vec![0.5, 0.6]), storage: StorageType::Float32x2 },
        ];
        let bytes = write_v2(&entries).unwrap();
        let (version, read_back) = read_binary(&bytes).unwrap();
        assert_eq!(version, 2);
