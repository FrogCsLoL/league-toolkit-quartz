...
const MATERIAL_NAMES: &[&str] = &[
    ...
    "MaterialOverride%N%BlendMode", "MaterialOverride%N%GlossTexture",
    "MaterialOverride%N%EmissiveTexture", "MaterialOverride%N%FixedAlphaScrolling",
    "MaterialOverride%N%Priority", "MaterialOverride%N%RenderingMode",
    "MaterialOverride%N%SubMesh", "MaterialOverride%N%Texture",
...
const PART_FIELD_NAMES: &[&str] = &[
    "field-accel-%N%", "field-attract-%N%",
    "field-drag-%N%", "field-noise-%N%",
    "field-orbit-%N%",
];
...
fn get_system_names() -> Vec<String> {
    let base: &[&str] = &[
        ...
        "GroupPart%N%", "GroupPart%N%Type", "GroupPart%N%Importance",
        "Override-Offset%N%", "Override-Rotation%N%", "Override-Scale%N%",
...
fn get_group_names() -> Vec<String> {
    let base: &[&str] = &[
        ...
        "ChildParticleName%N%", "ChildSpawnAtBone%N%", "ChildEmitOnDeath%N%",
    ];
...
fn get_fluid_names() -> Vec<String> {
    let base: &[&str] = &[
        ...
        "f-jetdir%N%", "f-jetdirdiff%N%",
        "f-jetpos%N%", "f-jetspeed%N%",
    ];
...
pub fn build_hash_map(group_names: &[String]) -> HashMap<u32, (String, String)> {
    ...
    for gn in group_names {
        add(&[gn.clone()], &group_field_names);
    }
    ...
    for gn in group_names {
        ...
        add(&[gn.clone()], &field_subgroup_names);
    }
    ...
    for gn in group_names {
        add(&[gn.clone()], &field_names);
        add(&[gn.clone()], &fluid_names);
    }
