...
pub fn read_ini(text: &str) -> Result<Troybin, TroybinError> {
    ...
    for (line_num, line) in text.lines().enumerate() {
        ...
        if in_unknown {
            ...
            let entry = RawEntry { hash, value, storage };
            ...
            let entry = RawEntry { hash, value: value.clone(), storage };
            raw_entries.push(entry);

            let section = sections.iter_mut().find(|s| s.name == *section_name).unwrap();
...
pub fn resolve_entries(raw: &[RawEntry]) -> (Vec<Section>, Vec<RawEntry>) {
    let hash_map = dictionary::resolve_entries(raw);
    ...
    // The dictionary builds entries in a specific order (system, groups, fields, fluids).
    // We iterate the hash_map directly for simplicity — the HashMap iteration order
    // is arbitrary, but we use the same "first match wins" logic by checking found_hashes.
    // To match the original tools' section ordering, we look up each raw entry in the map.
    for entry in raw {
        if found_hashes.contains(&entry.hash) { continue; }
    ...
    for entry in raw {
        if !found_hashes.contains(&entry.hash) {
            ...
            found_hashes.push(entry.hash); // don't duplicate
        }
    }

    // Drop unused hash_map binding
    let _ = hash_map;
